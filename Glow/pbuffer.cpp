...
PBuffer::PBuffer(int width,int height,int float_buffer) : width(width), height(height) {
	...
	if(!config) {
		std::cerr << "pbuffer: glXChooseFBConfigSGIX() failed\n";
		return;
	}
	...
	if(!pbuffer) {
		std::cerr << "pbuffer: glXCreateGLXPbufferSGIX() failed\n";
		return;
	}

	GLXContext context = glXCreateContextWithConfigSGIX(display,config[0],GLX_RGBA_TYPE,old_context,true);
	if(!context) {
		std::cerr << "pbuffer: glXCreateContextWithConfigSGIX() failed\n";
		return;
	}
...
int PBuffer::enable() {
	if(!glXMakeCurrent(data->display,data->pbuffer,data->context)) {
		std::cerr << "pbuffer: glXMakeCurrent() failed\n";
		return -1;
	}
	return 0;
}
...
PBuffer::PBuffer(int width,int height,int float_buffer) : width(width), height(height) {
	...
	if(!wglChoosePixelFormatARB) {
		std::cerr << "pbuffer: wglGetProcAddress(\"wglChoosePixelFormatARB\") failed\n";
		return;
	}
	...
	if(!wglCreatePbufferARB) {
		std::cerr << "pbuffer: wglGetProcAddress(\"wglCreatePbufferARB\") failed\n";
		return;
	}
	...
	if(!wglGetPbufferDCARB) {
		std::cerr << "pbuffer: wglGetProcAddress(\"wglGetPbufferDCARB\") failed\n";
		return;
	}
	...
	if(!wglReleasePbufferDCARB) {
		std::cerr << "pbuffer: wglGetProcAddress(\"wglReleasePbufferDCARB\") failed\n";
		return;
	}
	...
	if(!wglDestroyPbufferARB) {
		std::cerr << "pbuffer: wglGetProcAddress(\"wglDestroyPbufferARB\") failed\n";
		return;
	}
	...
	if(count == 0) {
		std::cerr << "pbuffer: wglChoosePixelFormatARB() failed\n";
		return;
	}
	...
	if(!pbuffer) {
		std::cerr << "pbuffer: wglCreatePbufferARB() failed\n";
		return;
	}

	HDC hdc = wglGetPbufferDCARB(pbuffer);
	if(!hdc) {
		std::cerr << "pbuffer: wglGetPbufferDCARB() failed\n";
		return;
	}

	HGLRC context = wglCreateContext(hdc);
	if(!context) {
		std::cerr << "pbuffer: wglCreateContext() failed\n";
		return;
	}

	if(!wglShareLists(old_context,context)) {
		std::cerr << "pbuffer: wglShareLists() failed\n";
		return;
	}
