...
void Font::load(const char *name) {
	...
	if(!strcmp(ext,".tga")) data = texture_load_tga(name,&size,&size);
	else if(!strcmp(ext,".jpg")) data = texture_load_jpeg(name,&size,&size);
	if(!data) {
...
/*
 */
void Font::printf(float width,float height,float x,float y,char *str,...) {
	char buf[1024];
	va_list argptr;
	va_start(argptr,str);
...
/*
 */
void Font::printf_center(float width,float height,float x,float y,char *str,...) {
	char buf[1024];
	va_list argptr;
	va_start(argptr,str);
	vsprintf(buf,str,argptr);
