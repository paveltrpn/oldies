...
#define CLIP(n,c0,c1,c2,c3) { \
	...
	frustum[n] *= 1.0 / vec3(frustum[n]).length(); \
}
...
int Mesh::checkBox(const vec3 &min,const vec3 &max) {
	for(int i = 0; i < 6; i++) {
		if(-vec3(frustum[i]).dot(vec3(min[0],min[1],min[2])) < frustum[i].w) continue;
		if(-vec3(frustum[i]).dot(vec3(min[0],min[1],max[2])) < frustum[i].w) continue;
		if(-vec3(frustum[i]).dot(vec3(min[0],max[1],min[2])) < frustum[i].w) continue;
		if(-vec3(frustum[i]).dot(vec3(min[0],max[1],max[2])) < frustum[i].w) continue;
		if(-vec3(frustum[i]).dot(vec3(max[0],min[1],min[2])) < frustum[i].w) continue;
		if(-vec3(frustum[i]).dot(vec3(max[0],min[1],max[2])) < frustum[i].w) continue;
		if(-vec3(frustum[i]).dot(vec3(max[0],max[1],min[2])) < frustum[i].w) continue;
		if(-vec3(frustum[i]).dot(vec3(max[0],max[1],max[2])) < frustum[i].w) continue;
		return -1;
	}
	return 0;
}
...
void Mesh::setTexture(int texture,GLuint texture_id,int s) {
	if(s < 0) for(int i = 0; i < (int)surface.size(); i++) surface[i]->texture_id[texture] = texture_id;
	else surface[s]->texture_id[texture] = texture_id;
}
