...
int GLApp::setVideoMode(int w,int h,int fs) {
	...
	if(!visualInfo) exit("couldn`t get an RGBA, double-buffered visual");
	...
	if(!glxContext) exit("glXCreateContext failed");
...
/*
 */
int GLApp::exit(const char *error,...) {
	if(error) {
		char buf[1024];
		va_list arg;
		va_start(arg,error);
...
int GLApp::setVideoMode(int w,int h,int fs) {
	...
	try {
		...
		if(!hwnd) throw("CreateWindowEx() fail");
		if(!(hdc = GetDC(hwnd))) throw("GetDC() fail");
		...
		if(!(wglcontext = wglCreateContext(hdc))) throw("wglCreateContext() fail");
		if(!wglMakeCurrent(hdc,wglcontext)) throw("wglMakeCurrent() fail");
