...
/*
 */
Texture::Texture(const char *name,int flag,int target) {
	load(name,flag,target);
}
...
/*
 */
void Texture::load(const char *name,int flag,int target) {
	int width,height;
	...
	if(flag & MIPMAP_SGIS) {
		glTexParameteri(texture_target,GL_GENERATE_MIPMAP_SGIS,GL_TRUE);
		if(texture_target == TEXTURE_1D) {
			glTexImage1D(texture_target,0,GL_RGBA,width,0,GL_RGBA,GL_UNSIGNED_BYTE,data);
		} else {
			glTexImage2D(texture_target,0,GL_RGBA,width,height,0,GL_RGBA,GL_UNSIGNED_BYTE,data);
		}
	} else {
		if(texture_target == TEXTURE_1D) {
			gluBuild1DMipmaps(texture_target,GL_RGBA,width,GL_RGBA,GL_UNSIGNED_BYTE,data);
		} else {
